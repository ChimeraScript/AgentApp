...
class UpdateTaskFragment : Fragment() {
    ...
    private fun setUp(){
        ...
        binding.UpdateTittleText.setText(args.task.tittle)
        binding.UpdateDescriptionText.setText(args.task.description)
        binding.UpdateSelectDate.setText(args.task.date)
        binding.UpdateSelectHour.setText(args.task.time)
        binding.UpdatePriority.setText(args.task.priority)
    ...
    private fun updateItem(){
        ...
        if(inputCheck(tittle, description, date, time, priority)){
            ...
            db.collection("users").document(prefs.getEmail())
                ...
                .collection("Tasks").document(args.task.id).update(
                    "Tittle" , tittle,
            ...
            WorkManager.getInstance(requireContext()).cancelAllWorkByTag(args.task.Creation)


            // Workmananger - new notification
            val data = dataNotification(tittle, description, args.task.Creation.toInt())
            worker.saveNotification(requireContext(),duration,data,args.task.Creation)


            Toast.makeText(requireContext(), "Updated Successfully!", Toast.LENGTH_SHORT).show()

            val action = UpdateTaskFragmentDirections.actionUpdateTaskFragmentToInDashboardFragment(args.dashboardId,args.dashboardName)
    ...
    private fun deleteTask(){
        ...
        builder.setPositiveButton("yes") { _, _ ->
            deleteFireStoreData(args.task)
            Toast.makeText(context, "Successfully removed!", Toast.LENGTH_SHORT).show()
            val action = UpdateTaskFragmentDirections.actionUpdateTaskFragmentToInDashboardFragment(args.dashboardId,args.dashboardName)
            findNavController().navigate(action)
        }
        builder.setNegativeButton("No"){ _, _ ->}
        builder.setTitle("Delete ${args.task.tittle}?")
        builder.setMessage("${prefs.getName()} are you sure you want to delete ${args.task.tittle}?")
        builder.create().show()
    ...
    private fun deleteFireStoreData(data: TasksFS){
        db.collection("users").document(prefs.getEmail())
            .collection("Dashboards").document(args.dashboardId)
            .collection("Tasks").document(data.id).delete()
    }
